//! Opencode plugin summary generation.
...
#[derive(Debug, Clone)]
pub struct OpencodePluginSummary {
    pub name: String,
...
pub trait OpencodePluginSummaryProvider: std::fmt::Debug + Send + Sync {
    fn summarize(&self, plugin_dir: &Path) -> OpencodePluginSummary;
}
...
impl OpencodePluginSummaryProvider for FilesystemPluginSummaryProvider {
    fn summarize(&self, plugin_dir: &Path) -> OpencodePluginSummary {
        ...
        let missing_hook_files = components
            ...
            .iter()
            .filter(|file| {
                !file.exists && file.source == OpencodePluginComponentPathSource::Manifest
            })
